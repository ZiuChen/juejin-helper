...
class LotteriesTask extends Task {
  ...
  async run(growthTask, dipLuckyTask) {
    ...
    const getProbabilityOfWinning = (sumPoint) => {
      ...
      const luckyValue =
        ((sumPoint + supplyPoint) / pointCost) * luckyValueCost + dipLuckyTask.luckyValue
...
成长API事件埋点: ${this.sdkTask.calledTrackGrowthEvent ? '调用成功' : '调用失败'}
OnLoad事件埋点: ${this.sdkTask.calledTrackOnloadEvent ? '调用成功' : '调用失败'}
模拟访问掘金首页: ${this.mockVisitTask.homePageVisited ? '成功' : '失败'}
模拟访问掘金签到页: ${this.mockVisitTask.signInPageVisited ? '成功' : '失败'}
----------
...
module.exports = async function run(cookie) {
